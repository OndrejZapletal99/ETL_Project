...
class SqlManager:
    ...
    def __init__(self,server: str ,database: str ,odbc_driver: str):
        ...
        self.engine = None
        self.connection_string = f"mssql+pyodbc://@{self.server}/{self.database}?trusted_connection=yes&driver={self.odbc_driver}"
    ...
    def execute_query_to_df(self,after_select_text: str):
        ...
        if self.engine is None:
            print('No connection to database, please create some connection')
    ...
    def create_table(self,table_name: str, database: str ,columns_and_types: dict):
        ...
        try:
            ...
            temp_df = self.execute_query_to_df(f"TABLE_NAME FROM [{database}].INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{table_name}'")
            if not temp_df.empty:
                print('Table already exists, please select another method (Append,Overwrite,Truncate)')
            else:

                print('Table does not exist and it will be created....')
                columns_definiton = ", ".join([f'{col} {data_type}' for col, data_type in columns_and_types.items()])

                create_table_query = f"CREATE TABLE [{database}].[dbo].[{table_name}] ({columns_definiton})"

                self.execute_query(create_table_query)


                check_table_query = f" * FROM [{database}].INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{table_name}'"
